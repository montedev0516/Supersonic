...
app.controller('myCtrl', function($scope) {
	...
	$scope.input_fa[3] = ['Mach Number (M)'];
	...
	$scope.output_fa['nu'] = 'Prandtl-Meyer function (deg)';
	...
	$scope.calculate = function() {
		...
		if($scope.shape_id==0) {
			...
			$scope.result['nu'] = (Math.sqrt(($scope.gamma+1)/($scope.gamma-1))*Math.atan(Math.sqrt(($scope.gamma-1)*(Math.pow($scope.value['m'],2)-1)/($scope.gamma+1)))-Math.atan(Math.sqrt(Math.pow($scope.value['m'],2)-1)))*180/Math.PI;
		}
	}
